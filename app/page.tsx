      ...
      <div className="w-full max-w-[768px] lg:max-w-none">
        ...
        <section className="flex flex-col gap-9 px-4 py-12 lg:px-20 lg:py-[86px] lg:gap-12">
          <div className="flex flex-col gap-4 items-start w-full lg:flex-row lg:items-center lg:gap-4">
            ...
            <Link href="/services" className="bg-[#d8ff74] flex gap-4 h-[50px] items-center justify-center px-6 py-1 rounded-2xl w-full hover:bg-[#c8ef64] active:bg-[#b8df54] transition-colors cursor-pointer lg:hidden">
              <span className="font-semibold text-base text-[#161616]">Все службы</span>
            </Link>
            <Link href="/services" className="hidden lg:flex bg-[#d8ff74] gap-4 h-[50px] items-center pl-6 pr-1 py-1 rounded-2xl shrink-0 hover:bg-[#c8ef64] active:bg-[#b8df54] transition-colors cursor-pointer">
              <span className="font-semibold text-base text-[#161616] whitespace-nowrap">Все службы</span>
          ...
          <div className="grid grid-cols-2 gap-2 w-full lg:hidden">
            {services.map((service) => {
              const card = (
                <div className="bg-white flex flex-col items-start justify-between overflow-hidden p-4 rounded-3xl w-full h-auto min-h-[140px] transition-all duration-200 hover:shadow-md hover:-translate-y-1 cursor-pointer">
                  <p className="font-bold text-base leading-normal text-[#161616]">
                    {service.name}
                  </p>
                  <div
          ...
          {/* Desktop grid — equal 5 columns */}
          <div className="hidden lg:grid lg:grid-cols-5 gap-4 w-full">
            {services.map((service) => {
              const card = (
                <div className="bg-white flex flex-col h-[280px] items-start justify-between overflow-hidden p-6 rounded-[36px] transition-all duration-200 hover:shadow-md hover:-translate-y-1 cursor-pointer">
        ...
        <section className="flex flex-col gap-9 px-4 py-12 lg:px-20 lg:py-[86px] lg:gap-12">
          ...
          <div className="flex flex-col gap-2 items-start w-full lg:flex-row lg:gap-6">
            {benefits.map((benefit) => (
              ...
              >
                <img src={benefit.icon} alt="" className="absolute bottom-0 right-0 w-[186px] h-[186px] opacity-50" />
                <p className="font-bold text-2xl leading-normal text-[#161616] w-full relative z-10">{benefit.title}</p>
                <p className="font-medium text-base leading-[1.4] text-[#161616]/65 w-full relative z-10">{benefit.text}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="px-4 py-12 lg:px-20 lg:py-[86px]">
          <p className="text-lg leading-[1.4] text-[#161616] w-full lg:text-[48px] lg:leading-[1.1] lg:max-w-[1035px] lg:font-bold">
            <span className="text-[#64d3ff]">Сервис позволяет отслеживать</span>{" "}
            посылки по трек-номеру из любых стран мира. Вы можете узнать текущее
            местоположение отправления, историю его перемещения и статус доставки.
          </p>
        </section>
          ...
          <div className="bg-[#b8f9fc] flex flex-col gap-6 items-start overflow-hidden p-6 rounded-3xl w-full lg:rounded-[36px]">
            <div className="flex gap-2 items-center w-full lg:gap-6">
              <div className="flex gap-2 items-center flex-1">
                <img src="/icons/LockKey.svg" alt="" className="w-6 h-6 shrink-0" />
                <p className="flex-1 font-bold text-2xl leading-normal text-[#161616]">Защита данных</p>
              </div>
              <p className="hidden lg:block font-medium text-[10px] leading-normal text-[#161616]/65">Реестр №16-22-006365</p>
            </div>
            ...
            <p className="font-medium text-[10px] leading-normal text-[#161616]/65 lg:hidden">Реестр №16-22-006365</p>
          </div>
        </section>
            ...
            <div className="flex flex-col gap-6 items-start w-full lg:flex-row lg:justify-between lg:items-start">
              <div className="flex flex-col gap-6 items-start w-full lg:gap-9 lg:w-auto lg:flex-1">
                ...
                <div className="flex items-start justify-between w-full lg:justify-start lg:gap-12">
                  <div className="h-5 w-[62px] relative lg:h-8 lg:w-[99px]"><img src="/logos/visa.svg" alt="Visa" className="w-full h-full" /></div>
                  <div className="h-5 w-[112px] relative lg:h-8 lg:w-[179px]"><img src="/logos/mastercard.svg" alt="Mastercard" className="w-full h-full" /></div>
                  <div className="h-5 w-[73px] relative lg:h-8 lg:w-[117px]"><img src="/logos/mir.svg" alt="Мир" className="w-full h-full" /></div>
                </div>
              </div>
